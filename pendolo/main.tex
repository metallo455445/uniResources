...
\begin{equation}
    \omega=\sqrt{\dfrac{mgd}{\mathcal I}}\label{eq:omega}
\end{equation}
...
Per trovare il valore di $d$, ovvero la distanza tra il centro di rotazione del pendolo ed il suo centro di massa abbiamo innanzitutto preso con il metro la distanza tra il bordo di un estremo della sbarra ed il bordo esterno più vicino all'estremo di ogni foro ottenendo i risultati riportati in Tabella \ref{misure bordo bordo}\\ %mettere ref%
...
Abbiamo eseguito un test del~$\chi^2$ nel fit ottenuto in Figura~\ref{fig:10musre} ed è resultato pari a~1603.2 per~9 gradi di libertà (10~misure - 1~parametro libero). In queste condizioni ci aspetterebbe un~$\chi^2$ che in media vale~$\nu$ = $dof$ = 9 con deviazione standard $\sigma = \sqrt{2 \nu} \approx 4.2$. \'E evidente che il valore di~$\chi^2$ trovato sia incompatibile a quello aspettato; ad avvalorare la testi si può notare come nel grafico dei residui i punti più vicini al baricentro si distaccano da zero a più di $2\sigma$.\\
Abbiamo così deciso di provare a fare un nuovo fit, stavolta eliminando la misura più vicina al baricentro. Il fit è visibile in Figura~\ref{fig:9misure}{}. Il test del~$\chi^2$ risulta uguale a~$104$ e cioè ancora incompatibile con il valore di aspettazione ovvero~$8\pm4$. Dal grafico dei residui emerge come le misure non fluttuino randomicamente attorno lo zero, probabilmente per la prima misura che anche questa volta supera i $2\sigma$. Si è quindi fatto un ulteriore fit(Figura~\ref{fig:8misure}) stavolta eliminando le 2 misure più vicine al baricentro. Di questo ultimo fit si può notare come i residui siano distribuiti come vorremmo (ovvero randomicamente attorno lo~0 e a non più di~$2\sigma$ a meno dell'ultima misura) ma il~$\chi^2$ ottenuto (ovvero 21.0) è anche questa volta superiore al valore di aspettazione~$7\pm3.7$.
...
\begin{figure}
    ...
    \resizebox{0.85\textwidth}{!}{%  <-- Ridimensiona all'80% della larghezza del testo
        \input{pendolo_10_definitivp.pgf}
    }
...
\begin{figure}
    ...
    \sffamily
    \resizebox{0.85\textwidth}{!}{%  <-- Ridimensiona all'80% della larghezza del testo
        \input{pendolo_9_definitivp.pgf}
    }
...
\begin{figure}
    \centering
    \resizebox{0.85\textwidth}{!}{%  <-- Ridimensiona all'80% della larghezza del testo
        \input{pendolo_8_definitivp.pgf}
    }
...
\begin{enumerate}
    \item Non è stato scelto rigorosamente un angolo~$\vartheta$ (rispetto alla posizione di riposo) adeguato per l'approssimazione %Non penso sia adeguato come termine%
